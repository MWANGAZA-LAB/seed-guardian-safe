...
CREATE INDEX IF NOT EXISTS idx_proof_of_life_wallet_verified 
...
CREATE INDEX IF NOT EXISTS idx_audit_logs_wallet_action 
...
CREATE INDEX IF NOT EXISTS idx_recovery_attempts_active 
...
CREATE INDEX IF NOT EXISTS idx_guardians_full_name_gin 
ON public.guardians USING gin(to_tsvector('english', full_name));

-- Add indexes for JSONB fields that might be queried
CREATE INDEX IF NOT EXISTS idx_recovery_attempts_data_gin 
ON public.recovery_attempts USING gin(recovery_data);

CREATE INDEX IF NOT EXISTS idx_proof_of_life_data_gin 
